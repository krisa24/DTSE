...
--the optimiser will transform the IN to the = when IN has just one element in it. So all OK    
...
--we do not have to cast it to init, BSEG.GJAHR = '2020' perform better
    ...
    AND BSEG.DMBTR*-1 >= 0
--BSEG.DMBTR <= 0 is the same equivalent of the conditions, performs better
...
/*
no need for nested Query, can be rewritten as
INNER JOIN  "DTAG_DEV_CSBI_CELONIS_DATA"."dtag.dev.csbi.celonis.data.elog::V_LFA1"
     ON 1=1 
     AND TEMP.LIFNR > '020000000'
*/
